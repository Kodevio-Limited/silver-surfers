...
function resolveChromePath() {
    const configuredPath = process.env.CHROME_PATH || process.env.CHROMIUM_PATH;
    if (configuredPath) {
        return configuredPath;
    }

    // Prefer Chrome launcher auto-discovery so local development does not hard-fail
    // when CHROME_PATH is omitted from .env.
    try {
        if (typeof chromeLauncher.getChromePath === 'function') {
            const autoDetectedPath = chromeLauncher.getChromePath();
            if (autoDetectedPath) {
                return autoDetectedPath;
            }
        }
    } catch (error) {
        console.warn(`Chrome auto-discovery via chrome-launcher failed: ${error.message}`);
    }

    // Fall back to Puppeteer's managed browser if one is available locally.
    try {
        ...
        console.warn(`Chrome auto-discovery via Puppeteer failed: ${error.message}`);
    }

    throw new Error(
        'Unable to locate a Chrome/Chromium executable. Set CHROME_PATH or CHROMIUM_PATH, or install the Puppeteer-managed browser.'
    );
}
    ...
    try {
        ...
        if (cdpUrl) {
            try {
                ...
                port = Number(parsedCdpUrl.port);
            } catch (error) {
                throw new Error(`Invalid CDP URL format: ${error.message}`);
            }
