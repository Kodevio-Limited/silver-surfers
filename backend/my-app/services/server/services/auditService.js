...
export async function generateSummaryPDF(platformResults, outputPath) {
...
export const runFullAuditProcess = async (job) => {
  ...
  try {
    ...
    const reportsByPlatform = {}; // { device: [{ jsonReportPath, url, imagePaths, score, scoreCard }] }
    ...
    if (record) {
      ...
      if (allScorecards.length > 0) {
        ...
        await record.save().catch((err) => console.error('Failed to save aggregate scorecard:', err));
      }
    }
    ...
    try {
      ...
      // Give any in-flight cloud storage streams time to settle before local cleanup.
      console.log(`⏳ Waiting 10 seconds before cleanup to ensure cloud uploads complete...`);
    ...
    if (record) {
      if (sendResult?.success) {
        ...
        if (sendResult.storage) {
          ...
          if (sendResult.storage.provider === 's3' && sendResult.storage.bucket && sendResult.storage.prefix) {
            record.reportDirectory = `s3://${sendResult.storage.bucket}/${sendResult.storage.prefix}`;
          }
        }
      } else {
    ...
    // Cleanup the report folder directly instead of exposing an open delete endpoint.
    try {
      const cleanupResult = await cleanupFolder(finalReportFolder);
      console.log('Report folder cleaned up:', cleanupResult.cleanedPath);
    } catch (cleanupErr) {
...
export const runQuickScanProcess = async (job) => {
    ...
    try {
        ...
        try {
          ...
          if (job.quickScanId && emailResult?.storage) {
            await QuickScan.findByIdAndUpdate(job.quickScanId, {
              ...
              console.error('Failed to persist quick scan storage metadata:', updateErr);
            });
          }
        ...
        const quickScanCacheMinutes = Math.max(1, Math.round((Number(process.env.QUICK_SCAN_REPORT_TTL_MS) || (30 * 60 * 1000)) / 60000));
        console.log(`🗂️ Quick scan report cached locally for ${quickScanCacheMinutes} minute(s) at: ${userSpecificOutputDir}`);
        ...
        return {
          ...
          reportDirectory: emailResult?.storage?.provider === 's3' && emailResult.storage.bucket && emailResult.storage.prefix
            ? `s3://${emailResult.storage.bucket}/${emailResult.storage.prefix}`
