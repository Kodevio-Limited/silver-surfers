...
async function getDriveClient() {
  ...
  driveInitializationPromise = (async () => {
    const hasDriveCredentials = process.env.GOOGLE_DRIVE_CLIENT_ID
    ...
    try {
      ...
      console.warn('Google Drive client could not be initialized:', error?.message || error);
      return null;
    }
  })();
...
function buildDriveStorageMetadata(uploadedFiles) {
  return {
    provider: 'google-drive',
    objectCount: uploadedFiles.length,
    objects: uploadedFiles.map((file) => ({
...
async function uploadFilesToConfiguredStorage(files, options = {}) {
  if (isS3Configured()) {
    ...
    const linksExpire = result.urlMode === 'signed';
  ...
  for (const file of files) {
    try {
      try {
        ...
        storageErrors.push(`${file.filename}: File was deleted before upload`);
        continue;
      }

      const uploadResult = await uploadToDrive(file.path, file.filename, options.folderPath, options.recipientEmail);
      uploadedFiles.push({
        filename: file.filename,
        size: file.size,
        sizeMB: file.sizeMB,
        downloadUrl: uploadResult.url,
        providerUrl: uploadResult.url,
      ...
      if (uploadError.code === 'ENOENT') {
        ...
        storageErrors.push(`${file.filename}: ${uploadError.message}`);
      }
    }
  }
  ...
  return {
    ...
    linksExpire: false,
    storage: buildDriveStorageMetadata(uploadedFiles),
    uploadedFiles,
    storageErrors,
  };
}
...
export async function sendAuditReportEmail({ to, subject, text, folderPath, isQuickScan = false, websiteUrl = '', quickScanScore = null, deviceFilter = null }) {
  ...
  console.log(`☁️ Uploading all files to ${isS3Configured() ? 'AWS S3' : 'Google Drive'} and sending download links`);

  try {
    const uploadResult = await uploadFilesToConfiguredStorage(files, {
      folderPath,
      recipientEmail: to,
      kind: isQuickScan ? 'quick-scans' : 'audit-reports',
    });

    uploadedFiles = uploadResult.uploadedFiles || [];
    storageErrors = uploadResult.storageErrors || [];
    storageMetadata = uploadResult.storage;

    if (storageMetadata?.provider === 's3' && storageMetadata.bucket && storageMetadata.prefix) {
      console.log(`🪣 Uploaded reports to ${buildS3Uri(storageMetadata.bucket, storageMetadata.prefix)}`);
    }
  } catch (storageError) {
    console.error('❌ Cloud storage upload failed:', storageError);
    storageErrors.push(storageError?.message || String(storageError));
  }
  ...
  if (uploadedFiles.length > 0) {
    ...
    if (storageMetadata?.provider === 's3' && storageMetadata.urlMode === 'signed') {
  ...
  try {
    ...
    return {
      success: true,
      attachmentCount: uploadedFiles.length || 0,
...
export async function sendMailWithFallback({ from, to, subject, html, text, attachments }) {
  ...
  // Process attachments if provided - upload to the configured cloud storage provider
  if (attachments && attachments.length > 0) {
    ...
    console.log(`☁️ Uploading all attachments to ${isS3Configured() ? 'AWS S3' : 'Google Drive'}`);
    ...
    for (const attachment of attachments) {
      if (attachment.path) {
        ...
        cloudFiles.push({
          ...
          size: await getFileSize(attachment.path),
          sizeMB: (await getFileSize(attachment.path) / (1024 * 1024)).toFixed(2),
        });
      } else {
    ...
    try {
      const uploadResult = await uploadFilesToConfiguredStorage(cloudFiles, {
        recipientEmail: to,
        kind: 'mail-assets',
      });
      uploadedFiles = uploadResult.uploadedFiles || [];
      ...
      console.error('❌ Failed to upload email attachments:', uploadError.message);
    }
    ...
    if (uploadedFiles.length > 0) {
      const downloadSection = '\n\n📁 DOWNLOAD LINKS FOR YOUR FILES:\n' +
        ...
        (storageMetadata?.provider === 's3' && storageMetadata.urlMode === 'signed'
          ? `\n⚠️ Note: Download links expire in ${S3_EXPIRY_DAYS} day${S3_EXPIRY_DAYS === 1 ? '' : 's'} for security.\n`
          : '\n');
      ...
      if (html) {
        html += '<br><br><h3>📁 Download Links for Your Files</h3>' +
          ...
          (storageMetadata?.provider === 's3' && storageMetadata.urlMode === 'signed'
            ? `<p><strong>⚠️ Note:</strong> Download links expire in ${S3_EXPIRY_DAYS} day${S3_EXPIRY_DAYS === 1 ? '' : 's'} for security.</p>`
            : '');
      }
    }
  }
  ...
  try {
    ...
    return { 
      success: true,
      attachmentCount: uploadedFiles.length || processedAttachments.length,
